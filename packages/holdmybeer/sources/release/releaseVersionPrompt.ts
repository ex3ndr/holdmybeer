...
import { type ReleaseVersionIncrementKind, releaseVersionIncrement } from "@/release/releaseVersionIncrement.js";
...
export async function releaseVersionPrompt(
    ...
    const customVersion = await releaseVersionCustomResolve(customVersionArg, nextVersions.patch);
...
async function releaseVersionModeResolve(
    ...
    const modeInput =
        ...
        (await promptInput(textFormatKey("prompt_release_mode", { patch: patchVersion }), "patch"))
            .trim()
            .toLowerCase();
...
async function releaseVersionCustomResolve(
    ...
    const value = customVersionArg?.trim() || (await promptInput(text.prompt_release_custom!, patchVersion)).trim();
