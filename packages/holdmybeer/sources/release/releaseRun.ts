...
export async function releaseRun(): Promise<void> {
    ...
    const nextVersion = await releaseVersionPrompt(currentVersion, process.argv[2], process.argv[3]);
    ...
    try {
        ...
        releaseCommitHash = await releaseCommandOutput("git", ["rev-parse", "HEAD"], repositoryDirectory);
...
async function releaseCommandOutput(command: string, args: string[], cwd: string): Promise<string> {
...
async function releaseRollback(releaseCommitHash: string | null, tagName: string, tagCreated: boolean): Promise<void> {
...
async function releasePublish(): Promise<void> {
    ...
    const publishArgs = ["publish", "--access", "public", "--registry", npmRegistry, "--no-package-lock"];
