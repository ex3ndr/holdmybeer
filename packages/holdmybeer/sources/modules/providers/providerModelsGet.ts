...
function providerModelFromRpc(input: {
    ...
    const inputTypes = (input.input ?? []).map((entry) => entry.trim()).filter((entry) => entry.length > 0);
    ...
    return {
        ...
        contextWindow:
            typeof input.contextWindow === "number" && Number.isFinite(input.contextWindow)
                ...
                : undefined,
        maxTokens:
            typeof input.maxTokens === "number" && Number.isFinite(input.maxTokens) ? input.maxTokens : undefined,
