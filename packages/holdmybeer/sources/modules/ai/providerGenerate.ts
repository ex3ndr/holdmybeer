...
export async function providerGenerate(input: ProviderGenerateInput): Promise<ProviderGenerateResult> {
    ...
    const prompts = [input.prompt, [input.prompt, OUTPUT_RETRY_PROMPT].join("\n\n")];
...
function providerArgs(_providerId: ProviderId, prompt: string, model: string | undefined): string[] {
