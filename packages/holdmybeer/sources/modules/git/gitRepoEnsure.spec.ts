...
describe("gitRepoEnsure", () => {
    ...
    it("does nothing when directory is already a git repository", async () => {
        ...
        expect(commandRunMock).toHaveBeenCalledWith("git", ["rev-parse", "--is-inside-work-tree"], {
            cwd: "/tmp/project",
            allowFailure: true
        });
    });
    ...
    it("initializes git when directory is not a repository", async () => {
        ...
        expect(commandRunMock).toHaveBeenNthCalledWith(1, "git", ["rev-parse", "--is-inside-work-tree"], {
            cwd: "/tmp/project",
            allowFailure: true
        });
        expect(commandRunMock).toHaveBeenNthCalledWith(2, "git", ["init"], { cwd: "/tmp/project" });
    });
});
