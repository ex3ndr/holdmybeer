...
export function textFormat(template: string, values: TextFormatValues): string {
...
export function textFormatKey<K extends TextKey>(key: K, values: TextValuesForKey<K>): string {
...
export function beerLog<K extends TextKeysWithValues>(key: K, values: TextValuesForKey<K>): void;

/** Logs a catalog message with optional template substitution to log file only. */
export function beerLog(key: TextKey, values?: TextFormatValues): void {
    beerLogLine(values ? textFormat(text[key], values) : text[key]);
}

function beerLogFilePathResolve(): string {
    const projectPath = process.env.BEER_PROJECT_PATH ?? process.env.INIT_CWD ?? process.cwd();
